...
if username:
    ...
    sorted_genre_counts = dict(sorted(genre_counts.items(), key=lambda x: x[1]))

    sorted_genre_counts = dict(list(sorted_genre_counts.items())[:10])

    #sorted_genre_counts = dict(sorted(genre_counts.items(), key=lambda x: x[1]))[:10]
